...
function updatePageHead(category, brand, query) {
  ...
  if (query) {
    ...
    desc.textContent = `Toda a seleção DomRoping da ${brand}.`;
    crumb.textContent = brand;
    document.title = `${brand} — DomRoping`;
  } else {
    title.textContent = "Todos os produtos";
    desc.textContent = "Filtre por marca, categoria ou faixa de preço para encontrar exatamente o que precisa.";
...
function init() {
  ...
  // Alterar categoria ou marca pelo <select> navega para a URL correspondente,
  // assim cada combinação continua tendo uma rota própria e compartilhável.
  $("#filterCategory").addEventListener("change", () => {
    const p = new URLSearchParams();
    if ($("#filterCategory").value) p.set("cat", $("#filterCategory").value);
    if ($("#filterBrand").value) p.set("marca", $("#filterBrand").value);
